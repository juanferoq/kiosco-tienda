...
// ── HeyGen — subir audio y generar video ─────────────────────
app.post('/api/heygen/generate', async (req, res) => {
  ...
  try {
    ...
    // Paso 1: Subir el audio a HeyGen
    console.log('Subiendo audio a HeyGen...');
    ...
    const assetId = uploadData.data?.id || uploadData.data?.asset_id;
    if (!assetId) throw new Error('No se obtuvo asset_id: ' + JSON.stringify(uploadData));

    // Paso 2: Generar el video con el audio subido
    console.log('Generando video con asset_id:', assetId);
    ...
    console.log('Video generate response:', JSON.stringify(videoData));
    res.json(videoData);
  } catch (err) {
