...
app.get('/api/heygen/test', async (req, res) => {
  try {
    const { default: fetch } = await import('node-fetch');
    // Intentar generar un video de prueba con texto simple
    const response = await fetch('https://api.heygen.com/v2/video/generate', {
      ...
      body: JSON.stringify({
        video_inputs: [{
          ...
          voice: {
            type: 'text',
            input_text: 'Hola, soy tu asistente virtual.',
    ...
    const data = await response.json();
    res.json({ avatar_id: HEYGEN_AVATAR_ID, response: data });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
